...
export default function PostList({ posts }: Props) {
  ...
  useEffect(() => {
    if (category === 'all') {
      ...
      setFilteredPosts(posts.filter((post: Post) => post.category === category));
    }
  }, [category]);
    ...
    <>
      ...
      <div className="basis-1/6 text-right">
        <div className="text-center inline-block">
          <h5 className="mb-4 text-lg font-bold pb-1 px-2 border-b-2 border-orange-600">Category</h5>
          <ul>
            <li className={`mb-1 cursor-pointer hover:text-orange-400 ${category === 'all' && 'text-orange-600'}`} onClick={() => setCategory('all')}>
              All Posts
            </li>
            <li
              className={`mb-1 cursor-pointer hover:text-orange-400 ${category === 'my story' && 'text-orange-600'}`}
              onClick={() => setCategory('my story')}
            >
              my story
            </li>
            <li
              className={`mb-1 cursor-pointer hover:text-orange-400 ${category === 'frontend' && 'text-orange-600'}`}
              onClick={() => setCategory('frontend')}
            >
              frontend
            </li>
            <li
              className={`mb-1 cursor-pointer hover:text-orange-400 ${category === 'backend' && 'text-orange-600'}`}
              onClick={() => setCategory('backend')}
            >
              backend
            </li>
            <li
              className={`mb-1 cursor-pointer hover:text-orange-400 ${category === 'javascript' && 'text-orange-600'}`}
